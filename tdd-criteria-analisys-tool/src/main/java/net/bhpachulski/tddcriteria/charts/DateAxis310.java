...
/**
 * User: hansolo Date: 16.12.13 Time: 15:03
 */
public class DateAxis310 extends Axis<LocalDateTime> {
	private enum Interval {
		DECADE(ChronoUnit.DECADES, 1), YEAR(ChronoUnit.YEARS, 1), MONTH_6(ChronoUnit.MONTHS, 6), MONTH_3(
				ChronoUnit.MONTHS, 3), MONTH_1(ChronoUnit.MONTHS, 1), WEEK(ChronoUnit.WEEKS, 1), DAY(ChronoUnit.DAYS,
						1), HOUR_12(ChronoUnit.HOURS, 12), HOUR_6(ChronoUnit.HOURS, 6), HOUR_3(ChronoUnit.HOURS,
								3), HOUR_1(ChronoUnit.HOURS, 1), MINUTE_15(ChronoUnit.MINUTES, 15), MINUTE_5(
										ChronoUnit.MINUTES, 5), MINUTE_1(ChronoUnit.MINUTES, 1), SECOND_15(
												ChronoUnit.SECONDS, 15), SECOND_5(ChronoUnit.SECONDS, 5), SECOND_1(
														ChronoUnit.SECONDS, 1), MILLISECOND(ChronoUnit.MILLIS, 1);
	...
	public DateAxis310(final String AXIS_LABEL, final LocalDateTime LOWER_BOUND, final LocalDateTime UPPER_BOUND,
			final boolean AUTO_RANGING) {
		super();
	...
	protected void setRange(final Object RANGE, final boolean ANIMATED) {
		...
		if (ANIMATED) {
			...
			currentAnimationID = animator.animate(
					new KeyFrame(Duration.ZERO, new KeyValue(currentLowerBound, toMillis(oldLowerBound)),
							new KeyValue(currentUpperBound, toMillis(oldUpperBound))),
					new KeyFrame(Duration.millis(700), new KeyValue(currentLowerBound, toMillis(lower)),
	...
	public LocalDateTime getValueForDisplay(double displayPosition) {
		...
		if (getSide().isVertical()) {
			...
			// lowerBound
			instant = Instant.ofEpochMilli((long) ((displayPosition - getZeroPosition() - getHeight()) / -range * diff
			...
			instant = Instant.ofEpochMilli(
					(long) ((displayPosition - getZeroPosition()) / range * diff + currentLowerBound.get()));
		}
	...
	protected List<LocalDateTime> calculateTickValues(final double VALUE, final Object RANGE) {
		...
		if (evenDateList.size() > 2) {
			...
			if (secondDate.toEpochSecond(ZoneOffset.ofHours(0))
					- lower.toEpochSecond(ZoneOffset.ofHours(0)) < (thirdDate.toEpochSecond(ZoneOffset.ofHours(0))
							- secondDate.toEpochSecond(ZoneOffset.ofHours(0))) * 0.5) {
			...
			// bound.
			if (upper.toEpochSecond(ZoneOffset.ofHours(0))
					- lastDate.toEpochSecond(ZoneOffset.ofHours(0)) < ((lastDate.toEpochSecond(ZoneOffset.ofHours(0))
							- previousLastDate.toEpochSecond(ZoneOffset.ofHours(0))) * 0.5)) {
	...
	protected String getTickMarkLabel(LocalDateTime date) {
		...
		if (actualInterval.INTERVAL == ChronoUnit.YEARS && calendar.getMonthValue() == 0
				&& calendar.getDayOfMonth() == 1) {
		...
		if (dates.size() > 2) {
			...
			for (int i = 0; i < dates.size(); i++) {
				...
				case YEARS:
					...
					calendar = calendar.withHour(0).withMinute(0).withSecond(0).withNano(6000000);
					break;
				case MONTHS:
					// If its not the first or last date (lower and upper
					...
					calendar = calendar.withHour(0).withMinute(0).withSecond(0).withNano(5000000);
					break;
				case WEEKS:
					// Make weeks begin with first day of week?
					calendar = calendar.withHour(0).withMinute(0).withSecond(0).withNano(4000000);
					break;
				case DAYS:
					calendar = calendar.withHour(0).withMinute(0).withSecond(0).withNano(3000000);
					break;
				case HOURS:
					if (i != 0 && i != dates.size() - 1) {
